...
def annotate_neighborhood_sequences(ref_ng_info_list):
    """
    """
    error_file ="not_found_amrs.txt"
    myerror = open(error_file, 'w')
    ...
    for amr_info_list in ref_ng_info_list:
        ...
        for index, amr_info in enumerate(amr_info_list[1]):
            ...
            annotation_prefix = 'ref_'+ amr_name1 +str(index)
            ...
            if found:
                ...
                myerror.write(amr_name+' annotation not found in ref_contig: '+contig_name+" seq_info: "+str(seq_info)+'\n')
        if ref_amr_info_list:
        	...
        	myerror.write(amr_name+' no annotation was found in reference.\n')
...
def find_all_amrs_and_neighborhood(amr_sequences_file, genome_file, neighborhood_len = 1000, threshold = 95):
    ...
    for record in amr_list:
        if int(float(record.identity))>=threshold:
            #amr_name = record.amr_name.split('[')[0].split('|')[-1].strip().replace(' ','_')
            ...
            if target_amr and int(float(record.matched_length)/target_amr['length']*100)>=threshold:
                if target_amr['name'] not in amr_name_list:
                    #amr_file_name = ''.join(e for e in target_amr['name'] if e.isalpha() or e.isnumeric() or e=='_' or e=='-')
                    ...
                    #amr_file = create_fasta_file(target_amr['seq'], AMR_dir, '>'+target_amr['title']+'\n', target_amr['name'])
...
def main(args):
	"""
	"""
	if args.seq and args.db:
		ref_ng_info = find_all_amrs_and_neighborhood(args.seq, args.db)
